...
deposit_details %>%
  ...
  select(download_url) %>%
  .[[1]] %>%
  download.file(destfile = "data-raw/download_responses.xlsx")


deposited_final_survey_responses <- read_xlsx("data-raw/download_responses.xlsx")
...
# 
# final_survey_responses <- read_xlsx("data-raw/FINAL Open Data 2017 anon rawdata.xlsx")
# final_survey_responses <- final_survey_responses %>%
#   slice(-1) %>%
#   slice(-1)
# 
# colnames(deposited_final_survey_responses)
# colnames(final_survey_responses)
# 

## ======== Import survey question labels

final_survey_labels <- read_csv("data-raw/FINAL_survey-question-labels.csv")
...
id_tibble <-
  tibble(responses.column.names = colnames(final_survey_responses)) %>%
  mutate(q.group = gsub("[_].*", "", responses.column.names)) %>%
  group_by(q.group) %>%
  mutate(id = row_number()) %>%
  mutate(
